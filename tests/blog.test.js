...
describe('Testing Blog routes', () => {
    ...
    it('should comment on blog article',async()=>{
        ...
        const article=await chai.request(app).get('/api/articles/')
        const id=article.body[0]._id
        const res=await chai.request(app).post(`/api/articles/comment/${id}`).send({"comment":"Testing Comment"}).set('Authorization', token)
        expect(res.status).to.be.equal(200)
        expect(res.body).to.be.a('object')
    }),
    it('should like on blog article',async()=>{
        ...
        const article=await chai.request(app).get('/api/articles/')
        const id=article.body[0]._id
        const res=await chai.request(app).post(`/api/articles/like/${id}`).set('Authorization', token)
        expect(res.status).to.be.equal(200)
        expect(res.body).to.be.a('object')
    })
})